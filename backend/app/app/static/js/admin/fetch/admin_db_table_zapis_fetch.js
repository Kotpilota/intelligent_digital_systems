  ...
  for (let i = 0; i < a.length; i++) {
    if (a[i] == names) {
      for (let j = 1; j < data[names].length + 1; j++) {
        ...
        let type = data[names][j].type;
        console.log(data[names].length, j);

        if (fun == 1 && j >= data[names].length - 1) {
          document.querySelector(
            ".btn-osn"
          ).innerHTML += `<button class="btnform btn-rec">Добавить</button><button class="btnform  close">Закрыть</button>`;
        }
        if (fun == 2 && j >= data[names].length - 1) {
          document.querySelector(
            ".btn-osn"
          ).innerHTML += `<button class="btnform btn-save">Изменить</button><button class="btnform close">Закрыть</button>`;
        }
        if (v == "id" || v == "created_at" || v == "updated_at") {
          ...
          if (names == "files") {
            form.innerHTML +=
              '<div class="form-group" ><h3 class="form-label" style="display:none">' +
              v +
              "</h3>" +
            ...
            if (type == "datetime") {
              form.innerHTML +=
                '<div class="form-group" ><h3 class="form-label">' +
                v +
                '</h3><input  type="date" class="form-input inp-date" name="' +
                v +
                '"' +
                ">" +
                "</div>";
              ...
              form.innerHTML +=
                '<div class="form-group" ><h3 class="form-label">' +
                v +
                '</h3><input  type="text" class="form-input" name="' +
                v +
                '"' +
                ">" +
                "</div>";
